  ...
  const processImage = (imageSrc) => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Start with higher quality for compression
        let quality = 0.8;
        let attempts = 0;
        ...
        const tryCompress = (currentQuality) => {
          canvas.toBlob((blob) => {
            ...
            // Try larger quality adjustments
            if (blob.size > MAX_SIZE) {
              quality -= 0.05; // Reduce quality more aggressively
            } else if (blob.size < MIN_SIZE) {
              quality += 0.05; // Increase quality if the image is too small
            }
            ...
            if (attempts < MAX_ATTEMPTS) {
              ...
              resolve({ blob, size: blob.size, quality: Math.round(currentQuality * 100) });
            }
          }, "image/jpeg", quality);
        };
